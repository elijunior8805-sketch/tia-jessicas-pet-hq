...
export const JessiInboxPanel: React.FC<JessiInboxPanelProps> = ({
  ...
  const hoje = kpis?.hoje ?? 0;

  const resumoLinguagemNatural =
    aguardando > 0
      ? `Olá! Na Central de Mensagens temos ${aguardando} cliente(s) aguardando resposta${
          naoLidas > 0 ? ` e ${naoLidas} mensagem(ns) não lida(s)` : ""
        }. Priorizei os contatos com maior tempo de espera para agilizar seu atendimento.`
      : naoLidas > 0
      ? `Temos ${naoLidas} mensagem(ns) não lida(s) no momento. Todas as conversas em aberto estão em dia!`
      : `Excelente! Todas as conversas foram respondidas e o atendimento via WhatsApp está 100% em dia.`;
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#123F2A] via-[#1A5C3D] to-[#0E3322] text-white p-4 md:p-5 shadow-sm border border-[#C8A951]/40 relative overflow-hidden">
            ...
            <div>
              <div className="inline-flex items-center gap-2">
                <span className="font-display font-bold text-base text-white tracking-tight">
                  Jessi · Central Inteligente de Mensagens
                </span>
              ...
              <p className="text-xs text-white/70">
                Sincronizado às {ultimaAtualizacao} • Monitoramento de conversas e respostas em tempo real
              </p>
            </div>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-black/20 border border-white/10 text-xs md:text-sm text-white/95 leading-relaxed backdrop-blur-xs">
          <div className="flex items-start gap-2.5">
            ...
            <div className="space-y-1">
              <p className="font-medium text-[#F5E6BE]">Análise do Atendimento:</p>
              <p className="text-white/90 leading-snug">{resumoLinguagemNatural}</p>
            </div>
          </div>
        </div>

        {/* Sugestões e Ações Rápidas em Chips */}
        <div className="mt-3 flex flex-wrap gap-2 items-center">
          ...
          {aguardando > 0 && onFiltrarAguardando && (
            ...
            >
              ...
              Priorizar Aguardando ({aguardando})
            </Button>
          )}
          {naoLidas > 0 && onFiltrarNaoLidas && (
            <Button
              size="sm"
              variant="outline"
            ...
            >
              ...
              Ver Não Lidas ({naoLidas})
            </Button>
          )}
          <Button
            size="sm"
            variant="outline"
            onClick={() => handlePerguntar("Jessi, quem são os clientes que estão aguardando retorno há mais tempo?")}
            className="h-7 text-xs bg-white/10 hover:bg-white/20 text-white border-white/20 rounded-lg gap-1.5 shadow-2xs"
          >
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              placeholder="Pergunte à Jessi sobre mensagens, clientes ou sugestões de resposta..."
              value={inputMsg}
