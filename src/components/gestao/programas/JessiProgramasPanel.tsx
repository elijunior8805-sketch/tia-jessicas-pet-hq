...
interface JessiProgramasPanelProps {
  totalAtivos?: number;
  totalCreditosDisponiveis?: number;
  vencendo7Dias?: number;
  onFiltrarVencendo?: () => void;
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

export const JessiProgramasPanel: React.FC<JessiProgramasPanelProps> = ({
  totalAtivos = 0,
    ...
    try {
      const res = await processarMensagemFn({
        data: {
          mensagem: query,
          contexto: {
            origem: "clubinho_programas",
            totalAtivos,
            totalCreditosDisponiveis,
            vencendo7Dias,
          },
        },
      });
      ...
      console.error("Erro ao consultar a Jessi no Clubinho:", err);
      toast.error("Não foi possível processar a consulta.");
      setRespostaJessi(
        "Tive uma dificuldade temporária ao consultar os dados do Clubinho. Por favor, tente novamente."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const resumoLinguagemNatural =
    totalAtivos > 0
      ? `Temos ${totalAtivos} plano(s) do Clubinho ativos no momento, somando ${totalCreditosDisponiveis} crédito(s) de banho disponíveis para os pets. ${
          vencendo7Dias > 0
            ? `Atenção: ${vencendo7Dias} contrato(s) vencem nos próximos 7 dias e podem ser renovados com os tutores.`
            : "Todos os contratos estão com validade confortável e em dia."
        }`
      : "Nenhum contrato ativo no Clubinho no momento. Excelente oportunidade para oferecer nossos planos de cuidados aos tutores!";

  return (
    <div className="space-y-3 my-2 animate-in fade-in duration-300">
      {/* Box Principal da Jessi no Clubinho */}
      <div className="rounded-2xl bg-gradient-to-br from-[#123F2A] via-[#1A5C3D] to-[#0E3322] text-white p-4 md:p-5 shadow-sm border border-[#C8A951]/40 relative overflow-hidden">
            ...
            <div>
              <div className="inline-flex items-center gap-2">
                <span className="font-display font-bold text-base text-white tracking-tight">
                  Jessi · Inteligência do Clubinho
                </span>
              ...
              <p className="text-xs text-white/70">
                Sincronizado às {ultimaAtualizacao} • Gestão de créditos de banho, validades de 30 dias e renovações
              </p>
            </div>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-black/20 border border-white/10 text-xs md:text-sm text-white/95 leading-relaxed backdrop-blur-xs">
          <div className="flex items-start gap-2.5">
            ...
            <div className="space-y-1">
              <p className="font-medium text-[#F5E6BE]">Visão Geral do Clubinho:</p>
        ...
        <div className="mt-3 flex flex-wrap gap-2 items-center">
          ...
          {vencendo7Dias > 0 && onFiltrarVencendo && (
            <Button
              size="sm"
              variant="outline"
              onClick={onFiltrarVencendo}
            ...
            >
              ...
              Vencendo em 7 dias ({vencendo7Dias})
            </Button>
          )}
          <Button
            size="sm"
            variant="outline"
            onClick={() => handlePerguntar("Jessi, quais pets têm Clubinho ativo com créditos disponíveis?")}
          ...
          >
            ...
            Pets com créditos ativos
          </Button>
          <Button
            size="sm"
            variant="outline"
            onClick={() => handlePerguntar("Jessi, quais são os planos do Clubinho cadastrados no catálogo?")}
            className="h-7 text-xs bg-white/10 hover:bg-white/20 text-white border-white/20 rounded-lg gap-1.5 shadow-2xs"
          >
            <Gift className="h-3 w-3 text-[#C8A951]" />
            Catálogo de planos
          </Button>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              placeholder="Pergunte à Jessi sobre o Clubinho, créditos de banho, validades ou planos..."
              value={inputMsg}
