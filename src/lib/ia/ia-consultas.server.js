...
export async function buscarDadosAgenda(sb, filtros) {
    let query = sb
        .from("agendamentos")
        .select(`
      *,
...
function formatarTelefoneBusca(termo) {
    return termo.replace(/\D/g, ""); // Apenas números
}
export async function buscarClientesIA(sb, termo) {
    ...
    if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(termoOriginal)) {
        ...
        if (data)
            return createIAResponse({ source: 'buscar_clientes', data: [data] });
    }
    // 2. Busca em camadas
    ...
    if (telefoneBusca.length >= 8) {
        ...
        if (data && data.length > 0)
            return createIAResponse({ source: 'buscar_clientes', data });
    }
    ...
    if (dataNome && dataNome.length > 0)
        return createIAResponse({ source: 'buscar_clientes', data: dataNome });
    ...
    if (dataPets && dataPets.length > 0) {
        ...
        if (clientesDosPets.length > 0)
            return createIAResponse({ source: 'buscar_clientes', data: clientesDosPets });
    }
...
export async function buscarPetsDoClienteIA(sb, clienteId) {
...
export async function buscarServicosIA(sb, termo) {
...
export async function listarAtendimentosIA(sb, filtros) {
    let query = sb
        .from("atendimentos")
        .select(`
      *,
    ...
    `);
    if (filtros.pet_id)
        query = query.eq("pet_id", filtros.pet_id);
    if (filtros.finalizado !== undefined)
        query = query.eq("finalizado", filtros.finalizado);
    if (filtros.data && /^\d{4}-\d{2}-\d{2}$/.test(filtros.data))
        query = query.gte("data_inicio", `${filtros.data}T00:00:00`).lte("data_inicio", `${filtros.data}T23:59:59`);
...
export async function buscarDadosFinanceiros(sb, filtros) {
    const timezone = "America/Sao_Paulo";
    const now = new Date();
...
export async function buscarDisponibilidade(sb, params) {
    ...
    if (params.servico_id && !params.duracao_min) {
        ...
        if (servico)
            duracao = servico.duracao_min || 60;
    }
...
export async function consultarResumoOperacionalIA(sb) {
    const timezone = "America/Sao_Paulo";
    const hoje = new Intl.DateTimeFormat("en-CA", {
        timeZone: timezone,
        year: "numeric", month: "2-digit", day: "2-digit",
...
export async function consultarHistoricoPetIA(sb, petId) {
...
export async function analisarRiscoEvasaoIA(sb) {
    ...
    if (!atendimentos || atendimentos.length === 0)
        return createIAResponse({ source: 'analisar_risco_evasao', data: [] });
    const petStats = {};
    atendimentos.forEach(a => {
        if (!petStats[a.pet_id]) {
            petStats[a.pet_id] = {
                datas: [],
                nome: a.pets?.nome || 'Pet',
                tutor: a.pets?.clientes?.nome || 'Tutor'
            };
        }
...
export async function obterVisao360Cliente(sb, clienteId) {
...
export async function obterVisao360Pet(sb, petId) {
...
export async function consultarRiscoFaltaIA(sb, clienteId) {
    const { data: agendamentos, error } = await sb
        .from("agendamentos")
        .select("status, data")
        .eq("cliente_id", clienteId)
...
export async function criarFilaEsperaIA(sb, params) {
