...
import { AlertCircle, Calendar, CheckCircle2, ExternalLink, MessageCircle, Search, Trash2, TrendingDown, Wallet, XCircle, Sparkles } from "lucide-react";
import { toast } from "sonner";
import { useRealtimeFinanceiro } from "@/lib/use-realtime-financeiro";
import { JessiPagamentosPanel } from "@/components/financeiro/JessiPagamentosPanel";
import { JessiCobrarDialog } from "@/components/financeiro/JessiCobrarDialog";
...
function PagamentosAbertosPage() {
  ...
  const [somenteMultiplos, setSomenteMultiplos] = useState(false);
  ...
  const contagemPorCliente = useMemo(() => {
    ...
    for (const item of itens) {
      map[item.cliente_nome] = (map[item.cliente_nome] || 0) + 1;
    }
    return map;
  }, [itens]);

  const clientesComMultiplosCount = useMemo(() => {
    return Object.values(contagemPorCliente).filter((c) => c > 1).length;
  }, [contagemPorCliente]);

  const filtrados = useMemo(() => {
    let res = itens;
    if (somenteMultiplos) {
      res = res.filter((i) => (contagemPorCliente[i.cliente_nome] || 0) > 1);
    }
  ...
  }, [itens, busca, somenteMultiplos, contagemPorCliente]);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Contas a receber com destaque para atrasos e cobrança assistida pela Jessi.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (!somenteAtrasados) setSomenteMultiplos(false);
            }}
          >
          ...
          <Button
            variant={somenteMultiplos ? "default" : "outline"}
            onClick={() => {
              setSomenteMultiplos((v) => !v);
              if (!somenteMultiplos) setSomenteAtrasados(false);
            }}
          >
            <Sparkles className="w-4 h-4 mr-2 text-[#C8A951]" />
            {somenteMultiplos ? "Múltiplos débitos" : "Múltiplos débitos"}
          </Button>
          <Button
      ...
      {/* 1. Painel de Análise Proativa e Pergunte à Jessi */}
      <JessiPagamentosPanel
        resumo={resumo}
        totalClientes={Object.keys(contagemPorCliente).length}
        clientesComMultiplosDebitos={clientesComMultiplosCount}
        onFiltrarAtrasados={() => setSomenteAtrasados(true)}
        onFiltrarMultiplosDebitos={() => setSomenteMultiplos(true)}
                ...
                <CardTitle className="text-lg flex items-center gap-2">
                  ...
                  {somenteMultiplos && (
                    <Badge variant="outline" className="text-xs bg-amber-50 text-amber-900 border-amber-300">
                      Filtrando clientes com múltiplos débitos
                    </Badge>
                  )}
                </CardTitle>
              ...
              {query.isLoading ? (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {filtrados.map((p) => {
                        const qtdDebitosCliente = contagemPorCliente[p.cliente_nome] || 1;
                        return (
                          <TableRow key={p.id} className={p.dias_atraso > 0 ? "bg-destructive/5" : ""}>
                            ...
                            <TableCell className="text-right space-x-2">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                className="rounded-lg font-medium border-emerald-300 text-emerald-950 hover:bg-emerald-50"
                                disabled={!p.cliente_whatsapp || registrarMut.isPending}
                                onClick={() => setCobrancaJessi(p)}
                              >
                                <Sparkles className="w-3.5 h-3.5 mr-1 text-[#C8A951]" />
                                Cobrar
                              </Button>
                              <Button
                                size="sm"
      ...
      {/* Modal Inteligente da Jessi para Cobrança Individual e Consolidada */}
      <JessiCobrarDialog
        open={!!cobrancaJessi}
        onOpenChange={(open) => !open && setCobrancaJessi(null)}
        pagamento={cobrancaJessi}
        outrasPendenciasDoCliente={
          cobrancaJessi
            ? itens.filter((i) => i.cliente_nome === cobrancaJessi.cliente_nome && i.id !== cobrancaJessi.id)
            : []
        }
        onSuccess={() => query.refetch()}
      />

      {/* Modal de Baixa / Recebimento Integrado */}
      <BaixaPagamentoDialog
        open={!!recebendoJessi}
        onOpenChange={(open) => !open && setRecebendoJessi(null)}
        pagamento={
          recebendoJessi
            ? {
                id: recebendoJessi.id,
                valor_total: recebendoJessi.valor_total,
                valor_pago: recebendoJessi.valor_pago,
                status: recebendoJessi.status,
                cliente_nome: recebendoJessi.cliente_nome,
                descricao: recebendoJessi.pet_nome
                  ? `Atendimento do pet ${recebendoJessi.pet_nome}`
                  : undefined,
              }
            : null
        }
