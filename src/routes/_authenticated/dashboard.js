...
import { TrendingUp, Wallet, Sparkles, PawPrint, Receipt, Calendar, Search, Plus, LineChart as LineChartIcon, Clock, AlertCircle, RefreshCw, Coins, } from "lucide-react";
...
function valorRealExecutado(atendimento) {
...
function DashboardPage() {
    ...
    const recalc = useMutation({
        mutationFn: () => recalcFn({ data: undefined }),
        onSuccess: async (res) => {
        ...
        onError: (e) => toast.error(e?.message ?? "Falha ao recalcular agregados."),
    });
    ...
    const { data: auxData } = useQuery({
        ...
        queryFn: async () => {
            ...
            const serie = diasIntervalo.map((d) => {
                ...
                const val = seriesData?.filter((p) => p.data_referencia === key)
    ...
    const atendimentosHoje = proximos.filter((a) => a.data === format(hoje, "yyyy-MM-dd")).length;
    const aguardando = proximos.filter((a) => a.status === "aguardando");
    return (<PageShell>
      {/* ============ HERO BANNER PREMIUM ============ */}
      <div className="relative overflow-hidden rounded-3xl mb-6 border border-[oklch(0.30_0.06_155)]" style={{
            background: "linear-gradient(135deg, oklch(0.24 0.045 155) 0%, oklch(0.30 0.06 155) 55%, oklch(0.34 0.07 155) 100%)",
        ...
        <div className="pointer-events-none absolute inset-0 opacity-[0.14]" style={{
            background: "radial-gradient(circle at 88% 20%, oklch(0.85 0.11 82 / 0.9), transparent 40%), radial-gradient(circle at 100% 100%, oklch(0.78 0.11 82 / 0.6), transparent 45%)",
        }}/>
        {/* Marca d'água pet */}
        <PawPrint className="pointer-events-none absolute -right-6 -bottom-8 h-56 w-56 text-white/[0.06] rotate-[-18deg]" strokeWidth={1}/>
        <PawPrint className="pointer-events-none absolute right-24 top-6 h-16 w-16 text-white/[0.08] rotate-[22deg] hidden sm:block" strokeWidth={1}/>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div className="min-w-0">
              ...
              <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl leading-[1.05] font-semibold text-white tracking-tight mt-2">
                {greeting()},{" "}
                <span style={{
            background: "linear-gradient(120deg, oklch(0.92 0.09 82), oklch(0.78 0.11 82))",
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_auto] gap-3 lg:min-w-[460px]">
              <div className="relative min-w-0">
                ...
                <Input placeholder="Buscar cliente, pet, serviço…" className="pl-10 h-11 rounded-full bg-white/10 backdrop-blur border-white/20 text-white placeholder:text-white/60 focus-visible:ring-[oklch(0.78_0.11_82)] focus-visible:ring-offset-0"/>
              </div>
              <Link to="/agenda" search={{}} className="w-full sm:w-auto">
                <Button size="default" className="h-11 w-full sm:w-auto px-6 rounded-full gap-2 whitespace-nowrap font-semibold border-0 transition-all hover:-translate-y-[1px]" style={{
            background: "linear-gradient(135deg, oklch(0.85 0.11 82), oklch(0.72 0.12 78))",
            color: "oklch(0.22 0.04 60)",
            boxShadow: "0 8px 20px -8px oklch(0.55 0.12 78 / 0.7), inset 0 1px 0 oklch(0.95 0.08 82 / 0.5)",
        }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        <PeriodTabs period={period} onChange={setPeriod} periodos={periodos} customFrom={customFrom} customTo={customTo} setCustomFrom={setCustomFrom} setCustomTo={setCustomTo}/>
        <Button variant="outline" size="sm" className="rounded-full gap-2 border-[oklch(0.85_0.05_155)] hover:bg-[oklch(0.94_0.03_155)]" onClick={() => recalc.mutate()} disabled={recalc.isPending} title="Recalcula históricos e agregados a partir dos lançamentos atuais (admin)">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-4 mb-6">
        {kpis.map((k) => (<Card key={k.label} className="group relative overflow-hidden p-4 sm:p-5 rounded-2xl border-border/60 bg-card shadow-[0_1px_2px_oklch(0.25_0.05_155/0.05),0_4px_16px_-8px_oklch(0.30_0.08_155/0.10)] hover:shadow-[0_8px_28px_-12px_oklch(0.30_0.08_155/0.22)] hover:-translate-y-0.5 transition-all duration-300">
            {/* Faixa lateral colorida */}
            <span className="absolute left-0 top-0 bottom-0 w-[3px]" style={{ background: `linear-gradient(180deg, ${k.tone.chip}, ${k.tone.bar})` }}/>
            <div className="flex items-start justify-between gap-2.5 sm:gap-3">
              ...
              <div className="grid h-9 w-9 sm:h-10 sm:w-10 shrink-0 place-items-center rounded-xl transition-transform group-hover:scale-110" style={{ backgroundColor: k.tone.soft, color: k.tone.chip }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,65fr)_minmax(0,35fr)] gap-4">
        <Card className="p-5 sm:p-6 rounded-2xl border-border/60 bg-card shadow-[0_1px_2px_oklch(0.25_0.05_155/0.05),0_4px_16px_-8px_oklch(0.30_0.08_155/0.10)]">
          ...
          {auxData && auxData.serie.some((p) => p.valor > 0) ? (<div className="h-48 sm:h-60 lg:h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={auxData.serie} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid oklch(0.90 0.01 120)", background: "oklch(1 0 0)", boxShadow: "0 10px 30px -12px oklch(0.30 0.08 155 / 0.18)" }} labelStyle={{ color: "oklch(0.32 0.06 155)", fontWeight: 600 }} formatter={(v) => [brl(v), "Receita"]}/>
                  <Area type="monotone" dataKey="valor" stroke="oklch(0.45 0.11 155)" strokeWidth={2.5} fill="url(#fillReceita)" dot={{ r: 3.5, fill: "oklch(0.45 0.11 155)", strokeWidth: 2, stroke: "oklch(1 0 0)" }} activeDot={{ r: 6, fill: "oklch(0.78 0.11 82)", strokeWidth: 2, stroke: "oklch(1 0 0)" }}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>) : (<div className="min-h-[11rem] grid place-items-center py-8 rounded-xl bg-[oklch(0.97_0.01_120)]/50 border border-dashed border-border/60">
        ...
        <div className="space-y-4">
          {/* Atenção hoje */}
          {aguardando.length > 0 && (<Card className="p-4 rounded-2xl border-[oklch(0.85_0.10_82)] bg-[oklch(0.98_0.03_82)]/70">
          ...
          <Card className="p-5 sm:p-6 rounded-2xl border-border/60 bg-card shadow-[0_1px_2px_oklch(0.25_0.05_155/0.05),0_4px_16px_-8px_oklch(0.30_0.08_155/0.10)]">
            ...
            {proximos.length === 0 ? (<div className="min-h-[8rem] grid place-items-center py-4 rounded-xl bg-[oklch(0.97_0.01_120)]/50 border border-dashed border-border/60">
                ...
                return (<Link key={a.id} to="/agenda" search={{}} className="group flex gap-2.5 sm:gap-3 items-center rounded-xl p-2 sm:p-2.5 -mx-2 sm:-mx-2.5 hover:bg-[oklch(0.97_0.02_150)] transition-colors">
...
function PeriodTabs({ period, onChange, periodos, customFrom, customTo, setCustomFrom, setCustomTo, }) {
    ...
    return (<div className="mb-6">
      <div role="tablist" aria-label="Filtro de período" aria-orientation="horizontal" className="grid grid-cols-4 sm:inline-flex sm:w-auto items-center rounded-full bg-card p-1 border border-border/60 shadow-[0_1px_2px_oklch(0.25_0.05_155/0.05)]">
        {periodos.map(([k, longLabel, shortLabel], idx) => {
            const active = period === k;
            return (<button key={k} ref={(el) => {
                    tabsRef.current[idx] = el;
                }} type="button" role="tab" id={`period-tab-${k}`} aria-selected={active} aria-label={longLabel} tabIndex={active ? 0 : -1} onClick={() => onChange(k)} onKeyDown={onKeyDown} className={`h-9 px-2 sm:px-5 rounded-full text-xs sm:text-sm font-semibold transition-all text-center leading-none focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background ${active
      ...
      {period === "personalizado" && (<div className="flex flex-wrap items-center gap-2 mt-3">
          <label className="sr-only" htmlFor="period-custom-from">Data inicial</label>
          <Input id="period-custom-from" type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} className="h-9 w-full sm:w-auto rounded-full" aria-label="Data inicial do período personalizado"/>
          ...
          <Input id="period-custom-to" type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} className="h-9 w-full sm:w-auto rounded-full" aria-label="Data final do período personalizado"/>
        </div>)}
    </div>);
}
